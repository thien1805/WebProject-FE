...
export default function PatientProfileCard({ user }) {
  // init data from backend user (nếu có), fallback demo
  const initialProfile = {
    name: user?.name || user?.fullName || fallbackProfile.name,
    email: user?.email || fallbackProfile.email,
    phone: user?.phone || fallbackProfile.phone,
    city: user?.city || fallbackProfile.city,
    country: user?.country || fallbackProfile.country,
  };
  ...
  const handleSubmit = (e) => {
    ...
    // TODO: call API update profile + password ở đây
        ...
        <div className="pd-profile-main">
          ...
          <div>
            <h3 className="pd-profile-name">
              {isEditing ? "Edit profile" : profileForm.name}
            </h3>
